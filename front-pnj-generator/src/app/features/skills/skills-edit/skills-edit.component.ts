/**
 * COMPOSANT D'ÉDITION DES COMPETENCES
 * 
 * Ce composant gère à la fois la création et la édition d'une skill.
 * Il est utilisé sur deux routes différentes :
 * - /universes/:universeId/skills/new          <- Création
 * - /universes/:universeId/skills/:skillId/edit   <- Édition
 * 
 * Le mode est déterminé par la présence ou non du paramètre skillId dans la URL.
 */

import { Component, OnInit, inject } from '@angular/core';
import { CommonModule } from '@angular/common';
import { FormControl, FormGroup, ReactiveFormsModule, Validators } from '@angular/forms';
import { ActivatedRoute, Router } from '@angular/router';
import { Skill } from '../../../models/features/skill.models';
import { UniverseService } from '../../../services/universe.service';
import { SkillService } from '../../../services/features/skill.service';
import { UniverseContextService } from '../../../services/universe-context.service';
import { Universe } from '../../../models/universe.models';

@Component({
  selector: 'app-skills-edit',
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule],
  templateUrl: './skills-edit.component.html',
  styleUrl: './skills-edit.component.scss'
})
export class SkillsEditComponent implements OnInit {

  // ID de la skill (null si création, string si édition)
  skillId: string | null = null;

  // ID de la univers parent (toujours requis)
  universeId: string = '';

  // Détermine si on est en mode création ou édition
  isEditMode = false;

  // Indicateur de sauvegarde en cours
  isSaving = false;

  /**
   * FORMULAIRE RÉACTIF
   * 
   * Seul le champ 'name' est requis pour la V1.
   * Les autres champs sont optionnels.
   * 
   * Note : On ne met PAS universeId dans le formulaire car :
   * - Il n'est pas éditable par la utilisateur
   * - On le récupère depuis la URL
   * - On la ajoute manuellement lors de la sauvegarde
   */
  skillForm = new FormGroup({
    name: new FormControl('', { nonNullable: true, validators: [Validators.required] }),
    relatedCharacteristic: new FormControl('', { nonNullable: true }),
    description: new FormControl('', { nonNullable: true }),
    bonus: new FormControl('', { nonNullable: true }),
    malus: new FormControl('', { nonNullable: true }),
    prerequisites: new FormControl('', { nonNullable: true }),
  });

  // Injection des dépendances (style moderne Angular)
  private readonly route = inject(ActivatedRoute);
  private readonly router = inject(Router);
  private readonly skillService = inject(SkillService);
  private readonly universeService = inject(UniverseService);
  private readonly universeContextService = inject(UniverseContextService);

  // Univers parent chargé depuis le service
  universe!: Universe;

  ngOnInit(): void {
    /**
     * RÉCUPÉRATION DES PARAMÈTRES DE ROUTE
     * 
     * Structure de nos routes imbriquées :
     * /universes/:universeId                    <- UniverseDetailComponent
     *   └─ /skills                             <- (loadChildren charge WEAPONS_ROUTES)
     *       └─ ''                               <- SkillsShellComponent
     *           └─ /new                         <- SkillsEditComponent (ON EST ICI)
     *           └─ /:skillId/edit              <- SkillsEditComponent (OU ICI)
     * 
     * Pour récupérer :universeId, il faut remonter dans la hiérarchie des routes.
     * L'universeId est défini 2 niveaux au-dessus (dans UniverseDetailComponent).
     */

    // On récupère skillId de la route actuelle (sera null si on est sur /new)
    this.skillId = this.route.snapshot.paramMap.get('skillId');
    this.isEditMode = !!this.skillId;

    this.universeId = this.universeContextService.requireCurrentUniverseId();

    // Chargement de la univers parent
    this.loadUniverse();

    // Si on est en mode édition, on charge la skill à éditer
    if (this.isEditMode && this.skillId) {
      this.loadSkill();
    }
  }

  /**
   * Charge la univers parent depuis le service
   */
  private loadUniverse(): void {
    this.universeService.getUniverseById(this.universeId).subscribe({
      next: (universe) => {
        this.universe = universe;
        console.log('✅ Univers chargé :', universe.name);
      },
      error: (err) => {
        console.error('❌ Erreur lors du chargement de l\'univers :', err);
        // Si la univers n'existe pas, on retourne à la liste des univers
        this.router.navigate(['/universes']);
      }
    });
  }

  /**
   * Charge la skill à éditer depuis le service (mode édition uniquement)
   */
  private loadSkill(): void {
    if (!this.skillId) return;

    this.skillService.getSkillById(this.universeId,this.skillId).subscribe({
      next: (skill) => {
        console.log('✅ Arme chargée :', skill);
        // On remplit le formulaire avec les données de la skill
        this.skillForm.patchValue({
          name: skill.name,
          relatedCharacteristic: skill.relatedCharacteristic,
          description: skill.description ?? '',
          bonus: skill.bonus,
          malus: skill.malus,
          prerequisites: skill.prerequisites,
        });
      },
      error: (err) => {
        console.error('❌ Erreur lors du chargement de la skill :', err);
        // Si la skill n'existe pas, on retourne à la liste des skills
        this.router.navigate(['../..'], { relativeTo: this.route });
      }
    });
  }

  /**
   * SAUVEGARDE DE L'PROTECTION
   * 
   * Gère à la fois la création et la mise à jour.
   * Le mode est déterminé par isEditMode (basé sur la présence de skillId).
   */
  save(): void {
    // Validation du formulaire
    if (this.skillForm.invalid) {
      // On marque tous les champs comme "touched" pour afficher les erreurs
      this.skillForm.markAllAsTouched();
      console.warn('⚠️ Formulaire invalide !');
      return;
    }

    // Récupération des valeurs du formulaire
    const formValues = this.skillForm.getRawValue();

    /**
     * Construction de la objet Skill
     * 
     * Notes importantes :
     * - On ajoute manuellement universeId (récupéré de la URL, pas du form)
     * - En mode création, id sera vide (le backend générera un UUID)
     * - En mode édition, on garde la id existant
     * - Les valeurs null deviennent undefined (convention du modèle)
     */
    const skillData: Skill = {
      id: this.isEditMode ? this.skillId! : '',
      name: formValues.name,
      relatedCharacteristic: formValues.relatedCharacteristic,
      description: formValues.description,
      bonus: formValues.bonus,
      malus: formValues.malus,
      prerequisites: formValues.prerequisites,
      universeId: this.universeId // ⚠️ Crucial : on lie la skill à son univers
    };

    console.log('💾 Sauvegarde de l\'skill :', skillData);

    // Appel du service approprié selon le mode
    if (this.isEditMode && this.skillId) {
      // Mode édition : mise à jour
      this.skillService.updateSkill(this.universeId, skillData).subscribe({
        next: (updated) => {
          console.log('✅ Arme mise à jour avec succès :', updated, "this.router", this.router);
          // Retour à la liste des skills
          this.router.navigate(['../..'], { relativeTo: this.route });
        },
        error: (err) => {
          console.error('❌ Erreur lors de la mise à jour :', err);
        }
      });
    } else {
      // Mode création : création d'une nouvelle skill
      this.skillService.createSkill(this.universeId, skillData).subscribe({
        next: (created) => {
          console.log('✅ Arme créée avec succès :', created);
          // Retour à la liste des skills
          this.router.navigate(['..'], { relativeTo: this.route });
        },
        error: (err) => {
          console.error('❌ Erreur lors de la création :', err);
        }
      });
    }
  }

  /**
   * ANNULATION
   * 
   * Retourne à la liste des skills sans sauvegarder.
   * Navigation relative : ['..'] remonte d'un niveau dans la hiérarchie des routes.
   */
  cancel(): void {
    console.log('❌ Annulation de l\'édition');
    this.router.navigate(['../..'], { relativeTo: this.route });
  }
}
